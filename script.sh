...
envsubst < config/bucket-payload-json.env.sh > config/bucket-payload.json
...
then
    ...
    curl -XPUT $es_url/_snapshot/$repository_name -H 'Content-Type: application/json' -d @config/bucket-payload.json
fi
